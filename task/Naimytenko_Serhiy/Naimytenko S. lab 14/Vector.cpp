...
vector<Capabilities> Merger_vector(vector<Capabilities>);
...
int Vector()
{
    ...
    int size = Count_line(file_read);vector<Capabilities> Merger_vectors(vector<Capabilities>);
    ...
    Criterion(conteiner);
    Search(conteiner);
    cout << "Слияние векторов" << endl;
    conteiner = Merger_vector(conteiner);
    Output(conteiner);
    Funct funct;
    cout << "Сортировка вектора" << endl;
    sort(conteiner.begin(), conteiner.end(), funct);
    Output(conteiner);
...
vector<Capabilities> Merger_vector(vector<Capabilities> cont)
{

    int x,size;
    cout << "Введите размер нового вектора" << endl;
    cin >> size;
    vector<Capabilities> vect;
    for (size_t i = 0; i < size; i++) /**< за допомогою циклу пройдемо всі комірки в масиві*/
    {
        ...
        if (x == 0) /**< варіант 1*/
        {
            ...
            vect.push_back(Capabilities(capabilities, dynamically, name, 3 + i, 1999, 20, func, lang, new_el_e));
        }
        else if (x == 1) /**< варіант 2*/
        {
            ...
            vect.push_back(Capabilities(capabilities, dynamically, name, 46 + i, 1998, 10, func, lang, new_el_e));
        }
        else if (x == 2) /**< варіант 3*/
        {
            ...
            vect.push_back(new_el_c);
        }
    }
    int order;
    cout << "Введите номер єлемента после которого хотите вставить новый контейнер" << endl;
    cin >> order;
    vector<Capabilities> copy; 
    int size_c = cont.size() + size;
    if (order != cont.size())
    {
         for (int i = order,j=0,k=0;i<size_c;j++,k++, i++)
         {
             if (j == size)
             {
                 k = 0;
             ...
             if (i < cont.size())
             {
                 copy.push_back(cont[i]);
                 cont[i] = vect[j];  
             }
             if(i>=cont.size()&&j<size)
             {
                 cont.push_back(vect[j]);
             }
             if(j>=size)
             {
                 cont.push_back(copy[k]);
             }
 
         } 
    } 
    else
        for(int i = order, j = 0; i < size + cont.size(); j++, i++)
            cont[i] = vect[j];
    return cont;
}
